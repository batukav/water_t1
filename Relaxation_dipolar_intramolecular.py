...
f = open('Relaxation_dipolar_intramolecular_npt_all_100.dat','w')
log = open('Relaxation_dipolar_intramolecular_npt_all_100.log','w')
log2 = open('logfile_intra.log','w')
...
for traject in traj_fitx:
	...
	for k in range(len(naigua)):
	  ...
	  for j in range(len(naigua)):
	    ...
	    d0 = d20**0.5
	    if (raigua[j]==raigua[k]) and (naigua[j]!=naigua[k]):
